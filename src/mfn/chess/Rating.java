...
public class Rating {
    ...
    public static int rateMovability(int listLength, int depth, int material) {
        ...
        if (listLength==0) {//current side is in checkmate or stalemate
            if (!Chess.kingSafe()) {//if checkmate
        ...
        for (int i=0;i<64;i++) {
            switch (Chess.chessBoard[i/8][i%8]) {
                ...
                case "Q": counter+=queenBoard[i/8][i%8];
                    break;
                case "A": if (material>=1750) {counter+=kingMidBoard[i/8][i%8]; counter+=Chess.posibleA(Chess.kingPositionC).length()*10;} else
                {counter+=kingEndBoard[i/8][i%8]; counter+=Chess.posibleA(Chess.kingPositionC).length()*30;}
                    break;
            }
        }
        return counter;
